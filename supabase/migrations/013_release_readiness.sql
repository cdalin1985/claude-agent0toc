...
-- ============================================================
-- Adds explicit match-fee payment methods, forfeit tracking,
-- treasury source-of-truth views, and database-side declined
-- challenge forfeiture/reversal support.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM public.challenge_forfeiture_events
  ...
  SELECT position, rank1_since
  INTO v_challenger_previous_position, v_challenger_rank1_since
  FROM public.rankings
  WHERE player_id = v_challenge.challenger_id;

  SELECT position, rank1_since
  INTO v_forfeiting_previous_position, v_forfeiting_rank1_since
  FROM public.rankings
  ...
  VALUES
    (v_challenge.challenger_id, v_challenge.discipline),
    (v_challenge.challenged_id, v_challenge.discipline)
  ON CONFLICT (player_id, discipline) DO NOTHING;

  SELECT to_jsonb(stats)
  INTO v_challenger_season_before
  FROM public.player_season_stats stats
  WHERE stats.player_id = v_challenge.challenger_id;

  SELECT to_jsonb(stats)
  INTO v_forfeiting_season_before
  FROM public.player_season_stats stats
  WHERE stats.player_id = v_challenge.challenged_id;

  SELECT to_jsonb(stats)
  INTO v_challenger_discipline_before
  FROM public.player_discipline_stats stats
  WHERE stats.player_id = v_challenge.challenger_id
    AND stats.discipline = v_challenge.discipline;

  SELECT to_jsonb(stats)
  INTO v_forfeiting_discipline_before
  FROM public.player_discipline_stats stats
  WHERE stats.player_id = v_challenge.challenged_id
    AND stats.discipline = v_challenge.discipline;
  ...
  IF v_challenger_previous_position IS NOT NULL
  ...
  SELECT position
  INTO v_challenger_new_position
  FROM public.rankings
  WHERE player_id = v_challenge.challenger_id;

  SELECT position
  INTO v_forfeiting_new_position
  FROM public.rankings
  WHERE player_id = v_challenge.challenged_id;

  UPDATE public.player_season_stats
  SET wins = wins + 1,
      forfeit_wins = forfeit_wins + 1,
      ...
      best_rank_achieved = CASE
        ...
        ELSE best_rank_achieved
      END,
      updated_at = now()
  WHERE player_id = v_challenge.challenger_id;

  UPDATE public.player_season_stats
  SET forfeits = forfeits + 1,
      current_streak = 0,
      updated_at = now()
  WHERE player_id = v_challenge.challenged_id;
  ...
  SET wins = wins + 1,
      ...
      updated_at = now()
  WHERE player_id = v_challenge.challenger_id
    AND discipline = v_challenge.discipline;
  ...
  IF v_cooldown_hours > 0 THEN
    ...
    VALUES (
      ...
      'post_match',
      now() + make_interval(hours => v_cooldown_hours)
    )
  ...
  VALUES (
    ...
    'Discipline: ' || v_challenge.discipline ||
      '. Race to ' || v_challenge.race_length ||
  ...
  WITH inserted_notifications AS (
    ...
    VALUES
      (
        ...
        'challenge_forfeit_win',
        'Challenge won by forfeit',
        COALESCE(v_forfeiting_name, 'Your opponent') ||
          ' declined your ' || v_challenge.discipline ||
      ...
      (
        v_challenge.challenged_id,
        'challenge_forfeited',
        'Challenge declined as forfeit',
        'Declining ' || COALESCE(v_challenger_name, 'the challenger') ||
          '''s ' || v_challenge.discipline ||
          ' challenge was recorded as a forfeit. No match fee was charged.',
        p_challenge_id,
        'challenge'
      )
    RETURNING id
  ...
  VALUES (
    p_actor_profile_id,
    'challenge_decline_forfeit_applied',
    'challenge',
    p_challenge_id,
    jsonb_build_object(
  ...
  INSERT INTO public.challenge_forfeiture_events(
    ...
    challenger_previous_position,
    forfeiting_previous_position,
    challenger_new_position,
    forfeiting_new_position,
    cooldown_id,
    activity_event_id,
    notification_ids,
    metadata
  ...
  VALUES (
    ...
    v_challenge.challenged_id,
    v_challenge.status,
    v_challenger_previous_position,
    v_forfeiting_previous_position,
    v_challenger_new_position,
    v_forfeiting_new_position,
    v_cooldown_id,
    v_activity_event_id,
    v_notification_ids,
    jsonb_build_object(
...
BEGIN
  ...
  SELECT position
  INTO v_challenger_current_position
  FROM public.rankings
  WHERE player_id = v_event.challenger_id;

  SELECT position
  INTO v_forfeiting_current_position
  FROM public.rankings
  ...
  IF v_challenger_current_position IS DISTINCT FROM v_event.challenger_new_position
    ...
    RAISE EXCEPTION
      'Cannot automatically reverse challenge %, rankings changed after the forfeit',
      p_challenge_id;
  END IF;
  ...
  IF v_challenger_season_before IS NULL
     OR v_forfeiting_season_before IS NULL
     OR v_challenger_discipline_before IS NULL
     OR v_forfeiting_discipline_before IS NULL THEN
  ...
  v_expected_challenger_season_streak := CASE
    WHEN (v_challenger_season_before->>'current_streak')::integer >= 0 THEN
      (v_challenger_season_before->>'current_streak')::integer + 1
    ELSE 1
  END;

  v_expected_challenger_discipline_streak := CASE
    WHEN (v_challenger_discipline_before->>'current_streak')::integer >= 0 THEN
      (v_challenger_discipline_before->>'current_streak')::integer + 1
    ELSE 1
  END;
  ...
  v_expected_challenger_best_rank := CASE
    ...
    ELSE (v_challenger_season_before->>'best_rank_achieved')::integer
  END;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Cannot automatically reverse challenge %, challenger season stats changed after the forfeit', p_challenge_id;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Cannot automatically reverse challenge %, forfeiting player season stats changed after the forfeit', p_challenge_id;
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM public.player_discipline_stats
    WHERE player_id = v_event.challenger_id
    ...
    RAISE EXCEPTION 'Cannot automatically reverse challenge %, challenger discipline stats changed after the forfeit', p_challenge_id;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Cannot automatically reverse challenge %, forfeiting player discipline stats changed after the forfeit', p_challenge_id;
  END IF;
  ...
  IF v_event.challenger_previous_position IS NOT NULL
    ...
    UPDATE public.rankings
    SET previous_position = v_event.challenger_new_position,
        position = v_event.challenger_previous_position,
        rank1_since = (v_event.metadata->>'challenger_rank1_since_before')::timestamptz,
        updated_at = now()
    WHERE player_id = v_event.challenger_id;
    ...
    SET position = position - 1001,
        updated_at = now()
    WHERE position BETWEEN (1000 + v_event.challenger_new_position + 1)
      AND (1000 + v_event.challenger_previous_position);

    UPDATE public.rankings
    SET rank1_since = (v_event.metadata->>'forfeiting_rank1_since_before')::timestamptz,
        updated_at = now()
    WHERE player_id = v_event.forfeiting_player_id;
  END IF;
  ...
  SET forfeits = (v_forfeiting_season_before->>'forfeits')::integer,
      current_streak = (v_forfeiting_season_before->>'current_streak')::integer,
      updated_at = now()
  WHERE player_id = v_event.forfeiting_player_id;
  ...
  SET wins = (v_challenger_discipline_before->>'wins')::integer,
      ...
      best_streak = (v_challenger_discipline_before->>'best_streak')::integer,
      updated_at = now()
  WHERE player_id = v_event.challenger_id
    AND discipline = v_event.metadata->>'discipline';
  ...
  SET forfeits = (v_forfeiting_discipline_before->>'forfeits')::integer,
      current_streak = (v_forfeiting_discipline_before->>'current_streak')::integer,
      updated_at = now()
  WHERE player_id = v_event.forfeiting_player_id
    AND discipline = v_event.metadata->>'discipline';
  ...
  INTO v_challenger_name
  FROM public.players
  WHERE id = v_event.challenger_id;

  SELECT full_name
  INTO v_forfeiting_name
  FROM public.players
  WHERE id = v_event.forfeiting_player_id;
    ...
    VALUES
      (
        ...
        'An admin reversed the accidental decline. Your challenge against ' ||
          COALESCE(v_forfeiting_name, 'the challenged player') ||
          ' is pending again.',
        p_challenge_id,
        'challenge'
      ),
      (
        ...
        'An admin reversed the accidental decline. ' ||
          COALESCE(v_challenger_name, 'The challenger') ||
          '''s challenge is pending again.',
        p_challenge_id,
        'challenge'
      )
    RETURNING id
  ...
  SELECT COALESCE(array_agg(id), '{}'::uuid[])
  INTO v_reversal_notification_ids
  FROM inserted_notifications;

  UPDATE public.challenge_forfeiture_events
  SET reversed_at = now(),
      reversed_by_profile_id = p_actor_profile_id,
  ...
  VALUES (
    p_actor_profile_id,
    'challenge_decline_forfeit_reversed',
    'challenge',
    p_challenge_id,
    jsonb_build_object(
